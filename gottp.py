...
class gottp(EasyFrame):
	"""GUI style framework"""
	def __init__(self):
		EasyFrame.__init__(self, title = "2-Ply Toilet Paper Calculator for a Household", background = "lightblue",width = 800, height = 400)
		#Labels for the window 
		self.addLabel(text = "Got Enough Toilet Paper?", row = 0, column = 0, columnspan = 100, sticky = "NSEW", background = "white", foreground = "darkgreen", font="bold")
		self.addLabel(text = "Number of adult women:", row = 1, column = 0, columnspan = 10, background = "lightblue", foreground = "white", font="bold")
		self.addLabel(text = "Number of adult men:", row = 2, column = 0, columnspan = 10, background = "lightblue", foreground = "white", font="bold")
		self.addLabel(text = "Number of children (11 years and under):", row = 3, column = 0, columnspan = 10, background = "lightblue", foreground = "white", font="bold")
		self.addLabel(text = "How many rolls of toilet paper do you currently have in stock?", row = 4, column = 0, columnspan = 10, background = "lightblue", foreground = "white", font="bold")
		self.addLabel(text = "How many sheets per roll in stock (see the packaging for details or otherwise guess)", row = 5, column = 0, columnspan = 10, background = "lightblue", foreground = "white", font="bold")
	...
	def calculate(self):	
		...
		#Compute Inputs
		roll = str(sheetspr)
		rollph = str(sheetspr/((p*j*female)+(q*k*male)+(r*l*kids)))
		rstock = str((sheetspr/((p*j*female)+(q*k*male)+(r*l*kids))) * onhand)
		facts = ((sheetspr/((p*j*female)+(q*k*male)+(r*l*kids))) * onhand)
